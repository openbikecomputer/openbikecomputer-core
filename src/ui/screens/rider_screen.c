...
static int _create_text_entry(T_rider_menu_element *element, lv_obj_t *subpage, char *name, char *placeholder)
{
...
static int _create_drop_down_entry(T_rider_menu_element *element, lv_obj_t *subpage)
{
	...
	/* Create label in container */
    element->label = lv_label_create(element->container);
    lv_label_set_text(element->label, _("Gender:"));
    lv_obj_set_size(element->label, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
	lv_obj_update_layout(element->label);
...
static int _create_subpage(void)
{
	...
	T_rider_menu_element menu_entry[NB_ENTRY] = {0};

	log("Create subpage: %d\n", rider_screen.btn_cnt);

	/* Create the menu subpage */
    subpage = lv_menu_page_create(rider_screen.menu, NULL);
	fail_if_null(subpage, -1, "subpage is NULL\n");
	...
	ret = _create_text_entry(&menu_entry[0], subpage, _("First name:"), _("First name"));
	fail_if_negative(ret, -2, "_create_text_entry failed, return: %d\n", ret);

	ret = _create_text_entry(&menu_entry[1], subpage, _("Name:"), _("Name"));
	fail_if_negative(ret, -3, "_create_text_entry failed, return: %d\n", ret);

	ret = _create_drop_down_entry(&menu_entry[2], subpage);
	fail_if_negative(ret, -4, "_create_drop_down_entry failed, return: %d\n", ret);

	ret = _create_text_entry(&menu_entry[3], subpage, _("Height:"), _("Pilot height in cm"));
	fail_if_negative(ret, -5, "_create_text_entry failed, return: %d\n", ret);

	ret = _create_text_entry(&menu_entry[4], subpage, _("Weight:"), _("Pilot weight in Kg"));
	fail_if_negative(ret, -6, "_create_text_entry failed, return: %d\n", ret);
	...
	/* The entry 2 with the drop down doesn't need keyboard input */
    lv_obj_add_event_cb(menu_entry[3].data, _ta_event_cb, LV_EVENT_ALL, kb);
    lv_obj_add_event_cb(menu_entry[4].data, _ta_event_cb, LV_EVENT_ALL, kb);
    ...
    cont = lv_menu_cont_create(rider_screen.main_page);
    label = lv_label_create(cont);
    lv_label_set_text_fmt(label, "New rider%"LV_PRIu32, rider_screen.btn_cnt);
    lv_menu_set_load_page_event(rider_screen.menu, cont, subpage);
...
int rider_screen_enter(lv_obj_t *screen)
{
    ...
    rider_screen.menu = lv_menu_create(screen);

	/* Enable root back button to exit the menu */
	lv_menu_set_mode_root_back_button(rider_screen.menu, LV_MENU_ROOT_BACK_BUTTON_ENABLED);
    lv_obj_add_event_cb(rider_screen.menu, _back_button_handler, LV_EVENT_CLICKED, rider_screen.menu);

	/* Set size and center the menu */
    lv_obj_set_size(rider_screen.menu, lv_pct(100), lv_pct(100));
    lv_obj_align(rider_screen.menu, LV_ALIGN_TOP_MID, 0, 0);

	/* Make the rider screen menu background transparent and set the correct font */
	lv_obj_add_style(rider_screen.menu, styles_get_transp_bg_style(), LV_PART_MAIN | LV_STATE_DEFAULT);
	lv_obj_add_style(rider_screen.menu, styles_get_font_inter_regular_18(), LV_PART_MAIN | LV_STATE_DEFAULT);

	log("Create menu main page\n");
    /*Create a main page that will contain the rider list */
    rider_screen.main_page = lv_menu_page_create(rider_screen.menu, NULL);

	/* TODO Populate the menu with existing rider configuration */

	log("Create menu button\n");
    /* Create floating button to add new rider configuration  */
    lv_obj_t * float_btn = lv_button_create(rider_screen.main_page);
    ...
    lv_obj_add_event_cb(float_btn, _float_button_handler, LV_EVENT_CLICKED, rider_screen.menu);
    lv_obj_set_style_radius(float_btn, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_img_src(float_btn, LV_SYMBOL_PLUS, 0);

	log("Create menu button\n");
	/* Go to the main page */
    lv_menu_set_page(rider_screen.menu, rider_screen.main_page);
